...
const Counter = ({ end, suffix = "", prefix = "", duration = 2 }: CounterProps) => {
  ...
  const isNumber = !isNaN(Number(end));

  useEffect(() => {
    if (!isInView) return;
    if (!isNumber) return; // ⬅️ do not animate text
    ...
    const animate = (timestamp: number) => {
      ...
      setCount(Math.floor(easeOut * Number(end)));
  ...
  }, [isInView, end, duration, isNumber]);
...
const metrics = [
  ...
  { value: "Quality", suffix: "+", label: "All GSM and Golden" }, // 👈 text metric
              ...
              >
                ...
                >
                  ...
                  <div className={`flex-1 ${index % 2 === 0 ? '' : 'text-right'}`}>
                    ...
                    <div
                      className={`h-px bg-gradient-to-r from-amber-500/50 to-transparent mt-2 ${
